"use client";

import { Suspense } from "react";
import { Shield, Lock } from "lucide-react";
import { useSearchParams } from "next/navigation";
import { signInWithGoogleAction } from "@/app/actions/auth";

function AdminLoginContent() {
  const searchParams = useSearchParams();
  const error = searchParams.get("error");

  const handleLogin = () => {
    // LLamar al Server Action (solo pedimos redirección al admin sin scopes adicionales obligatorios por ahora)
    signInWithGoogleAction("/auth/admin", null);
  };

  return (
    <div className="w-full max-w-md">
      <div className="bg-neutral-900 border border-neutral-800 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
        {/* Decors */}
        <div className="absolute top-0 right-0 w-64 h-64 bg-blue-500/5 blur-[100px] rounded-full pointer-events-none"></div>

        <div className="flex flex-col items-center text-center mb-8 relative z-10">
          <div className="w-16 h-16 bg-blue-500/10 flex items-center justify-center rounded-2xl border border-blue-500/20 mb-6">
            <Shield className="w-8 h-8 text-blue-400" />
          </div>
          <h1 className="text-2xl font-bold text-white tracking-tight mb-2">Acceso Restringido</h1>
          <p className="text-neutral-400 text-sm">
            Panel exclusivo para el staff de R4TLABS. Solo cuentas autorizadas.
          </p>
        </div>

        {error === "AccessDenied" && (
          <div className="mb-6 p-4 bg-red-500/10 border border-red-500/20 rounded-xl flex items-start gap-3">
            <Lock className="w-5 h-5 text-red-400 shrink-0 mt-0.5" />
            <p className="text-sm text-red-400">
              Acceso denegado. Tu cuenta de correo no está autorizada para entrar al panel de administración.
            </p>
          </div>
        )}
        {error === "AuthFailed" && (
          <div className="mb-6 p-4 bg-red-500/10 border border-red-500/20 rounded-xl flex items-start gap-3">
            <Lock className="w-5 h-5 text-red-400 shrink-0 mt-0.5" />
            <p className="text-sm text-red-400">
              Ocurrió un error al intentar iniciar sesión con Google.
            </p>
          </div>
        )}

        <button
          onClick={handleLogin}
          className="w-full h-12 bg-white hover:bg-neutral-100 text-black font-semibold rounded-xl transition-colors flex items-center justify-center gap-3"
        >
          <svg viewBox="0 0 24 24" className="w-5 h-5">
            <path
              fill="currentColor"
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
            />
            <path
              fill="#34A853"
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
            />
            <path
              fill="#FBBC05"
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
            />
            <path
              fill="#EA4335"
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
            />
          </svg>
          Ingresar con Google
        </button>
      </div>
    </div>
  );
}

export default function AdminLogin() {
  return (
    <div className="min-h-screen bg-neutral-950 flex flex-col items-center justify-center p-6 selection:bg-blue-500/30">
      <Suspense fallback={<div className="text-white text-sm">Cargando...</div>}>
        <AdminLoginContent />
      </Suspense>
    </div>
  );
}
